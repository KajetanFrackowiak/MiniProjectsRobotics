...
class Planner(LeafSystem):
    def __init__(self, plant, rng: np.random.Generator):
    ...
    def Plan(self, context, state):
        ...
        if mode == PlannerState.PICKING_FROM_X_BIN:
            X_G["place"] = RigidTransform(
                ...
                [self._rng.uniform(-0.25, 0.15), self._rng.uniform(-0.6, -0.4), 0.3],
            )
        else:
            X_G["place"] = RigidTransform(
                RollPitchYaw(-np.pi / 2, 0, np.pi / 2),
                [self._rng.uniform(0.35, 0.65), self._rng.uniform(-0.12, 0.28), 0.3],
            )
