...
class OperationalStiffnessController(LeafSystem):
    def __init__(self, plant: MultibodyPlant, k_p: np.ndarray, k_d: np.ndarray) -> None:
        ...
        self._p_des_port = self.DeclareVectorInputPort(
            "p_des", 3
        )  # [x_des, z_des, theta_des]
        self._v_des_port = self.DeclareVectorInputPort(
            "v_des", 3
        )  # [x_dot_dex, z_dot_des, theta_dot_des]

        self._iiwa_state_port = self.DeclareVectorInputPort(
            "iiwa_state", 6
        )  # [q2, q4, q6, q2_dot, q4_dot, q6_dot]

        self.DeclareVectorOutputPort(
            "iiwa_torques", 3, self.CalcTorqueOutput
        )  # [tau_q2, tau_q4, tau_q6]
