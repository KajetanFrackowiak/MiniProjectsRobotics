...
from pydrake.multibody.tree import SpatialInertia, UnitInertia, PrismaticJoint, RevoluteJoint, FixedOffsetFrame, ModelInstanceIndex
from pydrake.math import RigidTransform
from manipulation import ConfigureParser

from utils.assets import PEG_URDF_PATH, TABLE_SDF_PATH, HOLE_SDF_PATH

def add_planar_peg_to_plant(plant: MultibodyPlant, peg_frame_offset: float = 0.0) -> ModelInstanceIndex:
    ...
    # Since in drake one joint always connects two frames (which in general belongs to two rigid bodies), so we cannot simply create:
    # world -> prismatic_x -> prismatic_z -> revolute_theta -> peg
    # and we have to connects them by fictitious rigid bodies (with zero mass and inertia)
    # world -> prismatic_x -> false_body1 -> prismatic_z -> false_body2 -> revolute_theta -> peg
    peg_false_body1 = plant.AddRigidBody(
        "false_body1",
        peg,
        # mass = 0, center of mass w.r.t body frame = [0, 0, 0], unit interia = [0, 0, 0] (Ixx, Iyy, Izz)
...
def add_planar_hole_to_plant(plant: MultibodyPlant, table: ModelInstanceIndex) -> ModelInstanceIndex:
