...
def run_iiwa_peg_simulation(
    ...
    builder.Connect(
        pos_ref.get_output_port(), controller.get_input_port(0)
    )  # [x, z, theta]
    builder.Connect(
        vel_ref.get_output_port(), controller.get_input_port(1)
    )  # [x_dot, z_dot, theta_dot]

    builder.Connect(
        station.GetOutputPort("iiwa.state_estimated"), controller.get_input_port(2)
    )
    ...
    plant.SetVelocities(
        plant_context, iiwa, np.zeros(plant.num_velocities(iiwa))
    )  # start at rest
